...
(() => {
  ...
  let model = localStorage.getItem(MODEL_STORE) || 'deepseek-v4-flash';
  let playerName = '';
  let gameId = '500wan';
  let country = '';
  let school = '';
  ...
  function systemPromptText() {
    ...
    if (g.type === 'exam') {
      return (
        ...
        '4. 学业模拟：成绩必须具体合理，并且总分、排名、单科必须与学校层次自洽——全国名校（镇海中学、人大附中、上海中学等）年级平均分很高，即使是年级靠后的学生总分通常也不低于约550-580分；重点高中基准中等偏高（约480-600分）；普通或薄弱学校基准相应较低（约350-500分）。禁止出现“名校+明显矛盾的低分”或“垫底名次却只有400多分”这类设定。满分参考上海高考：语数外各150分，加选考科目共750分左右。学生在校住读，场景都在校内展开。个人成绩随玩家的行动与努力合理波动，不得固定。\n' +
  ...
  async function startExamOpening() {
    ...
    showTyping();
    try {
      const userMsg = '请展示开局：按输出格式输出时间、资产、情况。情况中必须写明：你是' + (school || '上海一所重点高中') + '高三住读学生，2027年2月27日，距离高考100天，场景在校园；请写出最近一次期末统考总分与年级排名（明确“总分 XXX 分”）。总分、排名、单科必须与学校层次自洽：镇海中学这类全国名校即使开局偏弱，总分也通常不低于约550-580分；重点高中约480-600分；普通学校约350-500分。禁止名校配极低分、或名次与分数自相矛盾的设定，也不要固定用同一个分数。只输出一次开局，情况之后给出至少1个具体 [OPTION:] 初始选项。';
